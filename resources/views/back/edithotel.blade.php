...
@section('content')
    <form class="card card-header col-md-5" style='translateX:(-50%); margin:1% 0 0 28%; '
        action="{{ route('admin-update', $hotels) }}" method="post" enctype="multipart/form-data">
        ...
        @if ($hotels->picture)
            <div class="col-4">
                <div class="mb-3 img">
                    <img src="{{ asset('pictures/' . $hotels->picture) }}">
    ...
    @if ($hotels->picture)
        <form style='translateX:(-50%); margin:1% 0 0 28%; ' action="{{ route('admin-update', $hotels) }}" method="post">
            <button type="submit" class="btn btn-outline-danger" name="delete_picture" value="1">Delete
                Photo</button>@method('put')@csrf</form>
    @endif
@endsection
